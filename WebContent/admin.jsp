...
<body>
	...
	<form action="registrarProducto" method="post" onsubmit="return validarNombreProducto()">
		<label for="nombre">Nombre del Producto:</label> <input type="text"
			id="nombre" name="nombre" required><br> <label
			for="descripcion">Descripción:</label>
		<textarea id="descripcion" name="descripcion" required></textarea>
		<br> <label for="valor_carbono">Valor de Carbono:</label> <input
			type="number" id="valor_carbono" name="valor_carbono" required><br>

		<%
		if ("duplicado".equals(request.getParameter("error"))) {
		%>
		<p>¡Error! El producto ya está registrado.</p>
		<%
		}
		%>
